(() => {
  ...
  const TEXT = {
    ko: {
      ...
      aggregationRange: "집계 기간", range7: "최근 7일", range30: "최근 30일", hideNoneActor: "None 제외",
    ...
    en: {
      ...
      aggregationRange: "Aggregation period", range7: "Last 7 days", range30: "Last 30 days", hideNoneActor: "Exclude None",
  ...
  // A stale value can name a range the server no longer accepts, so fall back to the default.
  const cachedDashboardDays = () => Number(localStorage.getItem("cyber-dashboard-days")) === 7 ? 7 : 30;
  ...
  function renderDashboardRange() {
    $("#dashboard-range")
      ...
      .html([7, 30].map(days => `<option value="${days}"${days === state.dashboardDays ? " selected" : ""}>${esc(t(`range${days}`))}</option>`).join(""))
      .prop("hidden", false);
  }
  ...
  // Repaint only the blocks the control changed. A full render would unmount the CVE card and
  // the surrounding layout for data that did not move, and flash the loading skeleton over it.
  // Scope "actors" covers the None filter, which the server applies to that one breakdown.
  ...
  function bindEvents() {
    ...
    $(document).on("change", "#dashboard-range", function () {
      state.dashboardDays = Number($(this).val()) === 7 ? 7 : 30;
