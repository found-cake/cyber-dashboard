...
func (c *Client) Generate(ctx context.Context, request Request) (string, error) {
	...
	content, err := c.complete(ctx,
		`Return one JSON object with a concise summary field. Base every claim only on the supplied facts. Use exactly this JSON shape and value type: {"summary":"Concise factual summary"}.`,
		string(facts), 800)
	if err != nil {
		return "", err
	}
