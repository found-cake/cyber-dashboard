...
func (f *blockingFetcher) Fetch(context.Context, api.Source) (feed.Document, error) {
	<-f.release
	return feed.Document{}, nil
}
...
func TestCollectWarnsUser_whenAIAPIIsUnavailable(t *testing.T) {
	server, _, _ := newTestServer(t, &stubFetcher{document: feed.Document{Articles: []feed.FeedArticle{{
