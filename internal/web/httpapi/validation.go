...
func writeBadRequest(c *echo.Context, message string) error {
	korean := map[string]string{
		...
		"sort must be score, cvss, mentions, or firstSeen":                          "정렬 기준은 score, cvss, mentions 또는 firstSeen이어야 합니다",
		"offset must be a non-negative multiple of 100":                             "offset은 0 이상의 100 단위 정수여야 합니다",
		"revision must be an unsigned integer":                                      "revision은 0 이상의 정수여야 합니다",
		"revision is required after the first CVE page":                             "첫 CVE 페이지 이후에는 revision이 필요합니다",
		"offset exceeds the current CVE catalogue":                                  "offset이 현재 CVE 목록 범위를 벗어났습니다",
...
func writeAPIError(c *echo.Context, err error) error {
	...
	if errors.Is(err, feedstore.ErrNotFound) || errors.Is(err, report.ErrNotFound) || errors.Is(err, settings.ErrSourceNotFound) {
