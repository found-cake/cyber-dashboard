...
Cyber Dashboard는 보안 뉴스를 수집합니다. 수집한 뉴스는 로컬 위협 인텔리전스 작업 공간에 정리합니다. 활성화한 피드에서 기사를 가져오고 본문을 읽습니다. 언급된 CVE는 NVD 정보로 보강합니다. 등록한 OpenAI 호환 LLM은 기사를 분류하고 일간·주간·월간 요약을 생성합니다.
...
<div align="center">
  <img src="assets/dashboard_white.webp" alt="라이트 테마 Cyber Dashboard" width="49%">
  <img src="assets/dashboard.webp" alt="다크 테마 Cyber Dashboard" width="49%">
</div>
...
- 테마 설정은 브라우저 `localStorage`에만 저장되며, 처음에는 시스템 테마를 따릅니다.
...
포트만 변경하려면 `127.0.0.1:8081`처럼 루프백 주소는 유지하고 포트 번호만 바꾸는 것을 권장합니다. `CYBER_DASHBOARD_ADDR=0.0.0.0:<포트>`로 설정하면 모든 네트워크 인터페이스에서 연결을 받아 다른 장치가 접근할 수 있습니다. Cyber Dashboard에는 사용자 인증 기능이 없으므로 꼭 필요한 경우가 아니라면 `0.0.0.0`을 사용하지 말고, 공용 인터넷에는 절대 직접 노출하지 마세요.
